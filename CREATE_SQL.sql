...
CREATE TABLE documentos_indexados (
    id_drive          VARCHAR(50) PRIMARY KEY,
    nombre_compuesto  VARCHAR(300) NOT NULL,
    url_acceso        VARCHAR(300) NOT NULL,
    procesado         BOOLEAN NOT NULL DEFAULT FALSE,
    primera_carpeta   VARCHAR(100)
);
...
CREATE TABLE lineas_documento (
    ...
    id_drive       VARCHAR(50) NOT NULL REFERENCES documentos_indexados(id_drive) ON DELETE CASCADE,
...
CREATE TABLE indice_invertido_uni (
    ...
    PRIMARY KEY (palabra, id_linea, posicion)
);
